...
@HiltViewModel
class CreateAndEditTaskViewModel @Inject constructor(
    private val eventUseCases: EventUseCases,
    state: SavedStateHandle
) : ViewModel() {

    private val _currentEvent = MutableStateFlow(CreateTaskScreenState())
    val currentEvent = _currentEvent.asStateFlow()

    private val _eventFlow = MutableSharedFlow<UiEvent>()
    val eventFlow = _eventFlow.asSharedFlow()

    private var preEditTask: Event = Event()
    ...
    fun updateStartDate(value: Long) {
        ...
        _currentEvent.value = currentEvent.value.copy(
            startDate = value,
            isStartDateError = value + currentEvent.value.startTime <= today.timeInMillis
    ...
    fun updateStartTime(value: Long) {
        ...
        _currentEvent.value = currentEvent.value.copy(
            startTime = value,
            isStartDateError = currentEvent.value.startDate + value <= today.timeInMillis
        )
    }
                ...
                if (currentEvent.value.id == null) {
                    id = eventUseCases.createEventUseCase(
                        Event(
                            ...
                            endDateTime = Date(currentEvent.value.startDate + currentEvent.value.startTime),
                            allDay = currentEvent.value.allDay,
                            color = currentEvent.value.color,
                            type = Event.Types.TASK,
