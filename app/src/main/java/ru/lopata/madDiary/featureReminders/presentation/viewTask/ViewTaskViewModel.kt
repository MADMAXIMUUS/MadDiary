...
class ViewTaskViewModel @Inject constructor(
    ...
    private val _currentEvent = MutableStateFlow(ViewTaskScreenState())
    ...
    fun editEvent() {
        ...
        bundle.putParcelable(
            ...
            currentEvent.value.toEventRepeatNotificationAttachment()
        )
        viewModelScope.launch {
            _eventFlow.emit(
                UiEvent.Edit(
    ...
    fun updateCheckState(checked: Boolean) {
        ...
        viewModelScope.launch {
            eventUseCases.createEventUseCase(event = currentEvent.value.toEvent())
        }
    }
}